...
\filledresponse{Agreed, revised. Benchmarked against comparable resources (e.g., the Immune
Signatures Data Resource, \textit{Sci. Data} 2022), which limit Technical Validation to QC/batch/
imputation checks with no predictive-modeling benchmarks. Age and Sex Prediction is retained as a
pure QC screen (mislabeling check), dropping its secondary predictive-benchmark framing. The
subsection combining batch-effect correction with an antibody-responder prediction benchmark
(RNN/LSTM/GRU/Transformer models) is being cut down to its batch-effect visualization check only,
since the predictive-modeling comparison is exactly the ``proof of utility'' this comment says is
not required (and is also the subject of Reviewer 2 comment 5, on possible QN artifacts in the
reported AUCs). Cell Type Annotation is retained, as it checks internal consistency between two
modalities rather than demonstrating a downstream application. \textbf{[PENDING: the
batch-effect-only reanalysis is being rerun before this is reflected in \texttt{template.tex}.]}}
