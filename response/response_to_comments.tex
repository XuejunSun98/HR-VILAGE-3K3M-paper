...
(\texttt{xuejun72/HR-VILAGE-3K3M}) and its URL. (A formal DOI-based reference number for the
repository, per E11 below, will be inserted at the same location once the Hugging Face DOI is
minted.) The section then describes the repository's file and folder structure --- the top-level
\texttt{study\_meta.csv} index, the five per-study data folders (\texttt{bulk\_gene\_expr/},
\texttt{bulk\_gene\_expr\_raw/}, \texttt{single\_cell\_gene\_expr/}, \texttt{meta/},
\texttt{antibody/}), and the supporting reference files (\texttt{Meta\_variable\_description.csv},
