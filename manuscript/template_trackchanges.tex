...
\medskip
\addmark{Several existing resources partially address this need but remain limited in scope. The GitHub repository \texttt{urmi-21/COVID-19-RNA-Seq-datasets} provides study-level summary matrices for COVID-19 RNA-seq studies but not fully processed, integration-ready data. Rogers et al.~\citep{rogers2019} conducted a meta-analysis of 18 influenza microarray datasets but did not include RNA-seq or single-cell data, clinical outcomes, or a unified harmonization framework. The pan-vaccine study by Fourati et al.~\citep{fourati2022} profiled immune responses to 13 viral and bacterial pathogens but included only a limited number of respiratory viral vaccine samples (approximately 300 subjects) and bulk transcriptomic data alone. Closing these gaps also requires harmonizing across platforms and correcting batch effects, for which established approaches include ComBat~\citep{wang2017combat}, limma~\citep{ritchie2015limma}, and sva~\citep{leek2012sva} for microarray data; ComBat-seq~\citep{zhang2020combatseq} and svaseq~\citep{leek2014svaseq} for RNA-seq; Harmony~\citep{korsunsky2019harmony}, canonical correlation analysis~\citep{stuart2019comprehensive}, and mutual nearest neighbors~\citep{haghverdi2018mnn} for single-cell data; and Quantile Normalization~\citep{bolstad2003}, Training Distribution Matching~\citep{thompson2015}, and Bayesian control-sample approaches~\citep{tang2021rankin} for cross-platform integration. To date, no existing resource combines microarray, bulk RNA-seq, and single-cell RNA-seq data across a comparable number of respiratory viral studies with harmonized metadata, standardized outcome definitions, and defined pre- and post-exposure timelines.}
