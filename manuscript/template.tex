...
Acute respiratory viral infections, such as those caused by influenza, respiratory syncytial virus (RSV), human rhinovirus (HRV) and SARS-CoV-2, represent a significant global health burden~\citep{bender2024global,chow2023effects,hodinka2016respiratory,simoes2015challenges}. Vaccination remains the most effective strategy for preventing infection and reducing transmission~\citep{gracca2023both,yaugel2022role,nehme2011relative}. Both humoral and cellular immunity are essential for controlling viral infections, and their diversity and specificity depend on complex interactions among immune cells, motivating a deeper understanding of human cellular immune responses to antigens over time.

Natural infection studies frequently lack pre-infection baseline data and cannot control infection timing, whereas human inoculation and vaccine studies offer unique opportunities to observe immune responses within a structured timeline. Studies of blood leukocytes and nasal swabs have shown that both systemic and local cellular immunity are crucial in defending against respiratory viral infections~\citep{liu2016individualized,zaas2009gene,huang2011temporal,davenport2015transcriptomic,rosenheim2023sars,proud2008gene,muller2017development,habibi2020neutrophilic}, and extensive transcriptomic data from such studies have been deposited in public databases, including GEO, ImmPort Shared Data, and ArrayExpress~\citep{barrett2012ncbi,bhattacharya2018immport,parkinson2007arrayexpress}. However, the analytical power of any single cohort is limited, highlighting the need for integrated data analysis across multiple studies to support reusable research and the development of AI/ML models for understanding infectious diseases and informing treatment and vaccine development.

Integrating data across studies presents several challenges. First, public repositories host data at inconsistent processing levels---from raw to fully normalized---generated through distinct study-specific pipelines, leading to apparent batch effects. Second, metadata on outcomes (e.g., infection status, symptoms, antibody responses) is often fragmented across supplementary files and deposited separately from the expression data (e.g., in ImmPort rather than GEO), with subject-level antibody titers frequently missing; sample IDs in the metadata and expression files sometimes differ, making merging impossible without direct communication with study investigators~\citep{grewe2024mva,lee2022prior,furman2017expression}. Third, data are profiled across microarray, bulk RNA-seq, and scRNA-seq platforms and use inconsistent gene symbols, including autoformatting artifacts such as alias names substituted for official nomenclature~\citep{ziemann2016gene}. Finally, rigorous preprocessing evaluation is often lacking: some datasets show abnormal sample correlations or irregular, discretized expression values indicative of improper processing, and duplicate samples deposited under different studies require careful detection and removal.

Several existing resources partially address this need but remain limited in scope. ImmPort, underlying the Human Immunology Project Consortium (HIPC) and its ImmuneSpace portal, aggregates over 300 immunology studies across many modalities (flow cytometry, cytokines, CyTOF), but only a small subset are respiratory viral vaccination/inoculation studies with transcriptomic data; HR-VILAGE-3K3M draws expression data from ImmPort for only 3 of 66 studies (10 more use ImmPort solely for antibody titers), harmonized alongside GEO- and ArrayExpress-sourced data into one consistent, transcriptomics-focused resource. The GitHub repository \texttt{urmi-21/COVID-19-RNA-Seq-datasets} offers study-level summary matrices but not integration-ready data. Rogers et al.~\citep{rogers2019} meta-analyzed 18 influenza microarray datasets but excluded RNA-seq, single-cell data, and clinical outcomes. Fourati et al.~\citep{fourati2022} profiled responses to 13 pathogens but included only about 300 respiratory viral vaccine samples and bulk data alone. Integrating across such heterogeneous sources further requires correcting batch effects and platform differences, using approaches such as ComBat~\citep{wang2017combat}, limma~\citep{ritchie2015limma}, and sva~\citep{leek2012sva} for microarray and RNA-seq data; Harmony~\citep{korsunsky2019harmony} and related methods~\citep{stuart2019comprehensive,haghverdi2018mnn} for single-cell data; and Quantile Normalization~\citep{bolstad2003} and related cross-platform approaches~\citep{thompson2015,tang2021rankin,Sun2026CrossPlatform} for bridging microarray and RNA-seq. To date, no existing resource combines microarray, bulk RNA-seq, and single-cell RNA-seq data across a comparable number of respiratory viral studies with harmonized metadata, standardized outcome definitions, and defined pre- and post-exposure timelines.

To overcome these barriers, we created the Human Respiratory Viral Immunization Longitudinal Gene Expression (HR-VILAGE-3K3M) data repository, a curated benchmark dataset encompassing over 3,000 participants from 66 studies, including vaccination, inoculation, and post-vaccination infection studies. We aligned metadata and gene expression profiles from disparate sources into a unified, harmonized structure: resolving inconsistent sample identifiers and gene-symbol formatting artifacts, harmonizing gene annotations to official nomenclature, and standardizing outcome definitions across studies. Subject-level antibody titers missing from public repositories were obtained directly from study authors when necessary, and rigorous quality control was applied to identify and exclude duplicate samples and datasets exhibiting abnormal or unreliable sample-level correlations; full procedures are described in Methods.

\section{Methods}

%\subsection{Data collection and curation}
\noindent\textbf{Data collection and curation.}
The data collection workflow of HR-VILAGE-3K3M is summarized in Figure \ref{fig:rigel}a. HR-VILAGE-3K3M consists of three components: gene expression profiles, metadata, and raw antibody data. We previously established a collection of human blood and nasal swab gene expression datasets from viral respiratory inoculation and vaccination studies. These datasets were gathered using targeted search queries from three publicly available repositories: GEO, ImmPort, and ArrayExpress, as detailed in~\citep{lan2024curated}, which curated 55 datasets (18 inoculation, 37 vaccination; approximately 2,513 subjects) sourced from these repositories, without further data processing or harmonization. HR-VILAGE-3K3M extends this previous collection with additional recent studies, standardized metadata and outcome definitions across studies, and curated subject-level antibody measurements obtained directly from study authors, for a total of 66 studies and over 3,000 subjects, all delivered as a harmonized, ready-to-use data product rather than a list of source datasets. The metadata was obtained from GEO, ImmPort, and supplementary materials. Subject-level antibody titers were obtained from GEO, ImmPort, supplementary files, and by requesting them from study authors via email. The constituent microarray and RNA-seq studies are cited at their original sources here~\citep{zhai2015host,sparks2023influenza,nakaya2015systems,franco2013integrative,bucasas2011early,furman2013apoptosis,furman2014systems,furman2015cytomegalovirus,avey2020seasonal,avey2017multiple,thakar2015aging,zapata2019impact,tsang2014global,rinchai2022high,furman2017expression,blazkova2017multicenter,alpert2019clinically,tomic2019fluprint,forst2022vaccination,hockman2025obese,odak2024systems,lee2022heterologous,lee2022comprehensive,lee2025rare,lee2022mrna,lee2022prior,hennighausen2026impact,ryan2023systems,arunachalam2021systems,nakaya2011systems,obermoser2013systems,henn2013high,sarin2024impaired,lindeboom2024human,proud2008gene,davenport2015transcriptomic,muller2017development,habibi2020neutrophilic,liu2016individualized,jochems2018inflammation,watanabe2023time,drury2024multi,grewe2024mva,chang2024altered,chang2024network,lucchesi2025transcriptomic,wimmers2021single,aydillo2022transcriptome,thwaites2023early,cao2014differences,narang2018influenza,monaco2019rna,carre2021endoplasmic,rinchai2025temporal}. Accession numbers and repository links for all 66 constituent studies, including source-paper citations, are provided in Supplementary Table 1.

%\subsection{Data processing}
\noindent\textbf{Data processing.}
For bulk expression data, the profiling platforms and data processing states vary greatly across GEO (microarray and RNA-seq), ImmPort (microarray), and ArrayExpress (RNA-seq). GEO requires raw FASTQ files for RNA-seq but does not mandate raw microarray data. Advantageously, all Affymetrix microarray studies in HR-VILAGE-3K3M have raw CEL files. We thus processed them using a standardized pipeline that includes \texttt{RMA}  normalization \citep{irizarry2003summaries}, probe-to-gene symbol mapping, Quantile Normalization (QN, \citep{bolstad2003comparison}), and $\log_2(x)$ transformation. When multiple probes mapped to the same gene, the probe with the highest median expression was retained. For Illumina microarray data, raw IDAT files were rarely available, but most studies provided non-normalized intensity data, which were processed by mapping probes to gene symbols using Illumina annotation tables, followed by QN and $\log_2(x)$ transformation. For RNA-seq, we mapped Ensembl IDs to HGNC gene symbols and provided raw counts data. In cases where only FASTQ files were available and raw counts were not provided, we performed read alignment and quantification to generate count matrices. All final expression matrices are provided at the gene level. 

For scRNA-seq, the studies provided variable data formats, including h5ad files, 10x matrix, and RObject files, with or without the metadata. We thus first integrate the metadata into the Python AnnData object \citep{Virshup2024anndata}, merge the data from the same study, and align to have consistent gene identifiers and cell-level metadata. Then we process them with the standardized preprocessing pipeline in \texttt{scanpy}~\citep{wolf2018scanpy}, including removal of genes expressed in fewer than 10 cells, total-count normalization (target sum: 10,000), and \texttt{log1p} (i.e., $\log(1+x)$) transformation. A dataset label was added to observations for tracking provenance. All scRNA-seq datasets were saved in AnnData (.h5ad) format, as it has an efficient processing speed and relatively smaller size. For visualization, all datasets were concatenated with an inner join over a shared set of 13,589 genes, with normalization, \texttt{log1p} transformation, Principal Component Analysis (PCA), neighborhood graph construction, and Uniform Manifold Approximation and Projection (UMAP,~\citealp{mcinnes2018umap}) embedding.
...
We generated a rigorously processed meta-source benchmark expression dataset, HR-VILAGE-3K3M, comprising 3{,}178 participants and 14{,}136 expression profiles from 66 studies. It includes 462 participants (18–55 years) from 15 inoculation studies involving five viruses (H3N2, H1N1, HRV, RSV, and SARS-CoV-2), profiled at 1–23 time points. Additionally, it includes 2{,}412 participants (0.5–89+ years) from 47 influenza and COVID-19 vaccination studies, profiled at 1–22 time points, and 304 participants from four studies with mixed exposure, profiled at 2–7 time points. Figure \ref{fig:rigel}b shows the bulk RNA-seq and scRNA-seq distribution of sample collection times following vaccination and inoculation, with dense sampling in the first few days for inoculation, while vaccination samples span longer durations—up to 180 and 360 days—though at lower frequencies. For each study, we provided processed gene expression data, along with the corresponding metadata and subject-level antibody information. Regarding sample types, 38 studies (58\%) used whole blood, 23 (35\%) used human peripheral blood mononuclear cells (PBMCs), and five (8\%) used nasal swabs. Of these studies, 34 (52\%) were conducted using microarray platforms, 25 (38\%) using RNA-seq, and seven (11\%) using 10x Genomics. In terms of study type, 15 (23\%) were inoculation studies, 47 (71\%) were vaccine studies, and 4 (6\%) were mixed exposure studies, including post-vaccination infection, post-infection vaccination, and natural infection (Figure \ref{fig:rigel}c). HR-VILAGE-3K3M provides a user-friendly platform for researchers to analyze the temporal dynamics of systems immunity to viral infections. The data is hosted on Hugging Face, enabling efficient access through Python with support for parallelized loading and selective retrieval. We also provide custom data splits for flexible loading by study type, tissue, and more. Our dataset is publicly available on Hugging Face at \url{https://huggingface.co/datasets/xuejun72/HR-VILAGE-3K3M}.


\section{Data Records}

The complete HR-VILAGE-3K3M dataset described in this Data Descriptor is deposited in the Hugging Face Datasets repository \texttt{xuejun72/HR-VILAGE-3K3M}, available at \url{https://huggingface.co/datasets/xuejun72/HR-VILAGE-3K3M} (data citation: \citep{sun2026hrvilagedataset}). The repository is organized into one top-level study index, four per-study data folders, and a small set of supporting reference files. Raw (unprocessed) input files are not mirrored in this repository; where available, they can be obtained from the original source repository (GEO, ArrayExpress, or ImmPort) cited for each study in Supplementary Table 1.

\noindent\textbf{\texttt{study\_meta.csv}.} A single table with one row per study (66 rows) that serves as the entry point to the repository. Columns are: \texttt{study\_ID} (study identifier, matching the folder/file names below); \texttt{platform} and \texttt{platform\_detail} (microarray, RNA-seq, or scRNA-seq, with the specific array/sequencer); \texttt{study\_type} (vaccine, inoculation, or mixed exposure); \texttt{pathogen} and \texttt{vaccine}; \texttt{tissue}; \texttt{raw\_data} and \texttt{antibody} (flags indicating whether raw expression data and antibody measurements are available for that study); \texttt{num\_subjects}, \texttt{num\_observations}, \texttt{num\_cells}, and \texttt{num\_observed\_genes}; \texttt{number\_of\_timepoints median(min, max)} and \texttt{observed\_timepoints}; and \texttt{meta\_variable\_list}, listing the metadata columns available for that study.

\noindent\textbf{\texttt{bulk\_gene\_expr/}.} One CSV file per bulk (microarray or RNA-seq) study, named \texttt{<study\_ID>\_gene\_expr.csv}, containing processed, normalized expression values in sample-by-gene format: rows are sample identifiers (matching \texttt{geo\_accession}/\texttt{ID} in the corresponding \texttt{meta/} file) and columns are HGNC gene symbols. All files share the same 41,667 gene columns, so that files can be concatenated directly across studies.

\noindent\textbf{\texttt{single\_cell\_gene\_expr/}.} One AnnData object per scRNA-seq study (\texttt{<study\_ID>\_processed.h5ad}), with raw count matrices stored in \texttt{.X} and cell-level metadata (e.g., cell type, sample ID, timepoint) stored in \texttt{.obs}. A merged object, \texttt{combined.h5ad.gz}, concatenates all scRNA-seq studies over a shared set of genes and additionally includes PCA and UMAP embeddings and neighborhood-graph information for visualization.

\noindent\textbf{\texttt{meta/}.} One CSV file per study, named \texttt{<study\_ID>\_meta.csv}, row-aligned with the corresponding file in \texttt{bulk\_gene\_expr/}. All metadata files share a common column set (a subset of columns applies to any given study, per its \texttt{meta\_variable\_list} entry in \texttt{study\_meta.csv}); missing values are coded as \texttt{NA}. Columns include, among others: \texttt{geo\_accession} and \texttt{ID} (sample and subject identifiers), \texttt{study\_ID}, \texttt{tissue}, \texttt{platform}, \texttt{pathogen}, \texttt{timepoint} (numeric day relative to vaccination/inoculation, standardized across all bulk studies; populated for 12{,}512/13{,}515 (93\%) bulk samples, missing only for 5 of 66 studies whose original source did not report exact day-level timing) and \texttt{timepoint\_group} (the original study-specific visit label, e.g., ``V1D0''; populated as a fallback for studies without a numeric \texttt{timepoint}, and occasionally alongside it as a supplementary label), \texttt{infection\_group} and \texttt{symptom\_group} (inoculation-study outcomes), \texttt{MFC} and \texttt{responder} (vaccine-study outcomes), \texttt{sex}, \texttt{age} and \texttt{age\_group}, \texttt{vaccine} and \texttt{vaccine\_dose}, and additional demographic and clinical variables (e.g., \texttt{BMI}, \texttt{ethnicity}, \texttt{race}, \texttt{Medications}, \texttt{Current.smoker}). The complete list of column names and their definitions is provided in \texttt{Meta\_variable\_description.csv}.

\noindent\textbf{\texttt{antibody/}.} One CSV file per study with antibody data available, named \texttt{<study\_ID>\_antibody.csv}, containing raw antibody measurements (e.g., HAI titers for influenza studies; binding and neutralizing antibody levels for COVID-19 studies) indexed by the same sample/subject identifiers used in \texttt{meta/}, allowing direct integration with expression and metadata records.

\noindent\textbf{Supporting files.} \texttt{Meta\_variable\_description.csv} is a data dictionary defining every column name used across the \texttt{meta/} files. \texttt{example\_dataset/} contains a worked example (metadata, gene expression, and antibody files for study GSE194378) illustrating how the three file types are linked by sample identifier. \texttt{croissant.json} provides a machine-readable Croissant-format description of the repository for programmatic dataset loading. \texttt{relation.png} documents studies that share subjects or originate from the same source series, for users combining data across studies.

\section{Technical Validation}
In this section, we report quality-control and validation checks on HR-VILAGE-3K3M: a metadata-consistency screen for sex and age, an assessment of batch-effect correction across studies and platforms, and a cross-modality consistency check between paired bulk and single-cell data.

\subsection{Age and Sex Metadata Consistency Check}


As a quality-control (QC) screen for potential sample mislabeling, we evaluated how well baseline whole-blood transcriptomes predict two recorded metadata fields, sex and age, using microarray and RNA-seq datasets from HR-VILAGE-3K3M. Sex prediction used a literature-curated set of Y-chromosome marker genes \citep{ellis2018sex}, while age prediction leveraged genome-wide expression features within a cross-validation framework.

The analyses for sex prediction included 1{,}096 microarray and 646 RNA-seq baseline samples, using the analysis-ready (QN-normalized, and for RNA-seq, $\log_2(x+1)$-transformed) expression data described in Methods. Y-chromosome marker genes were selected from this data, retaining only those expressed in all samples to ensure consistent cross-platform coverage. For each sample, a composite Y-score was computed as the sum of normalized expression values across all selected Y-chromosome marker genes. The optimal threshold separating males and females was determined using Youden’s $J$ statistic\citep{youden1950index}, maximizing the difference between true-positive and false-positive rates (TPR – FPR) on the ROC curve. Samples with Y-scores above this threshold were classified as male, while those below were classified as female.

The analyses for age prediction included 1{,}116 microarray and 784 RNA-seq baseline samples, using the same analysis-ready expression data. Genes missing in any sample were removed, and low-information genes (bottom 25\% by mean and variance) were excluded to improve model stability. Age (in years) was used as the target variable. Within each cross-validation fold, the top 2{,}000 highly variable genes (HVGs) were selected from the training data \citep{yip2017evaluation}, two modeling strategies were applied: (1) Ridge regression with L2 regularization optimized via \texttt{RidgeCV} \citep{hoerl1970ridge,scikit-learn}, and (2) a Fully Connected Neural Network (FCNN) comprising two hidden layers (512 and 256 ReLU units), a dropout rate of 0.3, and the Adam optimizer (learning rate $1\times10^{-3}$) \citep{chollet2015keras,kingma2014adam}. Early stopping and learning rate scheduling were employed to mitigate overfitting. Models were evaluated using 10-fold cross-validation with out-of-fold predictions \citep{arlot2010survey}. Performance metrics included the coefficient of determination ($R^2$) and mean absolute error (MAE, years), providing a consistent framework for assessing both linear and nonlinear age-related expression patterns across modalities.
...
The Y-chromosome marker--based classifiers achieved near-perfect discrimination between recorded male and female samples, with ROC-AUC values of 0.989 for microarray and 0.991 for RNA-seq (Table 1), well above the chance-level baseline of AUC~=~0.5. For age prediction, models achieved \(R^2\) between 0.674 and 0.859 across modalities and model types (Table 2), well above the na\"ive baseline of predicting each subject's age as the training-set mean age ($R^2=0$ by construction). The high concordance between expression-based predictions and recorded metadata across both platforms indicates no widespread sex- or age-metadata mislabeling in HR-VILAGE-3K3M; samples with low-confidence predictions (Y-scores near the classification threshold, or large age-prediction residuals) can be used to flag individual candidates for further QC review.

\subsection{Batch-Effect Correction Assessment}

To assess whether harmonization brings samples from different studies and platforms into alignment while preserving biological signal, we evaluated batch-effect correction on a subset of HR-VILAGE-3K3M comprising 22 influenza vaccine studies with recorded antibody-responder status (both microarray and RNA-seq; studies with fewer than 5 subjects in the minority responder class were excluded), totaling 3,288 samples (2,711 microarray, 577 RNA-seq) at early post-vaccination time points (prior to day 28). RNA-seq data were log-transformed using $\log_2(x+1)$, and genes in the bottom 25\% of both mean expression and variance were removed to reduce technical noise.

We applied three correction methods---QN~\citep{bolstad2003comparison}, ComBat~\citep{johnson2007adjusting}, and Regression from \texttt{scanpy}~\citep{wolf2018scanpy}, benchmarked in~\citep{Sun2026CrossPlatform}---using study identifiers as the batch variable, and visualized the corrected data with t-SNE, colored by study, responder status, and timepoint (Figure~\ref{fig:batcheffect}). As a quantitative summary, we computed the fraction of variance across the top 10 principal components explained by study identity, before and after each correction.

\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{BatchEffect_Correction.png}
  \caption{Assessment of batch-effect correction across 22 influenza vaccine studies (3,288 samples; microarray and RNA-seq). Rows show t-SNE projections of the expression data after Quantile Normalization, ComBat, and Regression; columns show the same projection colored by (a) study identity, (b) antibody-responder status, and (c) timepoint (binned from the recorded day of sample collection). }
  \label{fig:batcheffect}
\end{figure}

\begin{table}[ht]
\centering
\caption{Fraction of variance (top 10 principal components, variance-weighted) explained by study identity, before and after batch-effect correction.}
\begin{tabular}{lc}
\hline
\textbf{Method} & \textbf{Variance explained by study} \\
\hline
Uncorrected & 0.818 \\
QN & 0.845 \\
ComBat & 0.0005 \\
...
\hline
\end{tabular}
\label{tab:batch_variance}
\end{table}

QN alone does not remove study-driven separation: study clusters remain visually distinct after QN (Figure~\ref{fig:batcheffect}, top row), and the fraction of variance explained by study identity is essentially unchanged from the uncorrected data (0.818 vs.\ 0.845; Table~\ref{tab:batch_variance}), consistent with QN acting on the marginal distribution of each sample rather than on study-level batch structure. ComBat and Regression both bring samples from different studies into close alignment (Figure~\ref{fig:batcheffect}, middle and bottom rows), reducing the variance explained by study identity to near zero (Table~\ref{tab:batch_variance}), while responder status and timepoint remain distributed similarly across the corrected space rather than collapsing into separate corrected-away clusters, indicating that batch correction does not trivially remove biological variation along with study-driven variation.

\subsection{Known Vaccination Response Signatures}

As a check that processing preserves expected biological signal, we tested for two well-established post-influenza-vaccination transcriptional signatures~\citep{nakaya2011systems,nakaya2015systems}: an interferon-stimulated gene (ISG) response at Day 1 and a plasmablast expansion signature at Day 7. For each subject with a baseline (Day 0) sample and a Day 1 or Day 7 sample, we computed a module score (mean of gene-wise $z$-scored expression across marker genes, using processed values from microarray and RNA-seq influenza vaccine studies) using literature-standard marker sets: ISGs \texttt{IFI27}, \texttt{IFI44L}, \texttt{ISG15}, \texttt{MX1}, \texttt{OAS1} for Day 1, and plasmablast markers \texttt{TNFRSF17}, \texttt{IRF4}, \texttt{XBP1}, \texttt{PRDM1}, \texttt{CD38} for Day 7, and compared each subject's post-vaccination score to their own baseline with a paired Wilcoxon signed-rank test.

\begin{table}[ht]
\centering
\caption{Known vaccination response signatures: paired within-subject module-score change from baseline.}
\begin{tabular}{lccc}
\hline
\textbf{Signature (day)} & \textbf{N paired subjects} & \textbf{\% subjects increased} & \textbf{Wilcoxon $p$} \\
\hline
Interferon-stimulated genes (Day 1) & 412 & 86.9\% & $2.9\times10^{-56}$ \\
Plasmablast markers (Day 7) & 713 & 81.3\% & $5.0\times10^{-67}$ \\
\hline
\end{tabular}
\label{tab:known_signatures}
\end{table}

Both signatures increased from baseline in the large majority of subjects (Table~\ref{tab:known_signatures}), consistent with the expected timing of the innate interferon response and the adaptive plasmablast expansion following influenza vaccination. Coverage across platforms is uneven for this comparison: RNA-seq contributes only 5 paired subjects at each timepoint (all remaining pairs are microarray), too few to test separately, so this check is presently powered primarily by the microarray-derived subset.

\subsection{Cell Type Annotation Using Paired Bulk and Single-Cell Data}


In this analysis, we investigated immune cell-type dynamics using paired bulk RNA-seq and scRNA-seq data from the same SARS-CoV-2 vaccine study, comparing PBMC samples collected around Day 1 and Day 7 post-vaccination from the bulk cohort (GSE201533, 43 subjects) and from a 6-subject scRNA-seq subset of that cohort (GSE201534, sampled at exactly Day 1 and Day 7); the two datasets are not matched at the individual-subject level; this is a cross-modality consistency check on the direction of cell-type trends, not a paired within-subject comparison.
...
All files can be downloaded directly from the Hugging Face repository and read with standard tools (e.g., \texttt{pandas} for the CSV files, \texttt{anndata} for the single-cell \texttt{.h5ad} files). We additionally provide a Python interface via the Hugging Face \texttt{datasets} package. \texttt{study\_meta.csv} can be loaded directly:

\begin{verbatim}
from datasets import load_dataset
repo = "xuejun72/HR-VILAGE-3K3M"
ds = load_dataset(repo, name="study_meta",
                   trust_remote_code=True, revision="script")
study_meta = ds["train"].to_pandas()
\end{verbatim}

The \texttt{meta/} and \texttt{bulk\_gene\_expr/} folders can each be loaded and concatenated across all studies in the same way (\texttt{name="meta"} or \texttt{name="bulk\_gene\_expr"}). Because studies differ in which metadata columns and platforms apply, an optional \texttt{split\_filter} argument restricts loading to a subset of studies matching criteria drawn from \texttt{study\_meta.csv} (e.g., \texttt{study\_type}, \texttt{platform}, \texttt{tissue}, \texttt{pathogen}, \texttt{vaccine}), avoiding the need to download the full repository:

\begin{verbatim}
split_filter = {"study_type": ["vaccine"],
                 "platform": ["RNA-seq"],
                 "tissue": ["PBMC", "nasal swab"]}
ds = load_dataset(repo, name="meta", trust_remote_code=True,
                   split_filter=split_filter, revision="script")
\end{verbatim}

Individual \texttt{antibody/} files, which differ in column structure across studies, should instead be loaded one study at a time via their file path:

\begin{verbatim}
load_dataset(repo, data_files="antibody/GSE194378_antibody.csv")
\end{verbatim}

\noindent Single-cell files are loaded with \texttt{anndata} after download (e.g.\ \texttt{ad.read\_h5ad(...)}), exposing raw counts in \texttt{.X} and cell-level metadata in \texttt{.obs}; the merged \texttt{combined.h5ad.gz} object additionally includes precomputed PCA/UMAP embeddings for immediate visualization across all single-cell studies. New users are encouraged to first inspect \texttt{example\_dataset/}, which provides matched metadata, gene expression, and antibody files for a single study (GSE194378) to illustrate how records are linked by sample identifier before working with the full repository. Complete, up-to-date loading examples, including code for reproducing the analyses in this manuscript, are maintained at \url{https://xuejunsun98.github.io/HR-VILAGE-3K3M/docs/index.html} and on the Hugging Face repository page.

\noindent HR-VILAGE-3K3M's scope is deliberately limited to gene expression and antibody-response data, harmonized uniformly across all 66 studies. Most constituent studies have no other modality available; a small number (13/66) have companion data at their original source (e.g.\ scRNA-seq, CITE-seq, autoantibody panels, flow cytometry, CyTOF, mass spectrometry) that is not included here, listed with accessions in Supplementary Table 2 for users who wish to pursue them directly.
...
\subsection{Batch-Effect Correction Assessment Details}

\textbf{Data and sample filtering.} We filtered studies based on biological relevance, retaining only influenza vaccine studies that included antibody measurements. Studies with fewer than 5 subjects in the minority responder class were excluded to ensure reliable estimation. Additionally, we restricted the dataset to samples collected within the early immune response window (before day 28) and removed samples from control or placebo arms.
